- [IPv6 IPoE transix IPv4 (DS-Lite)の設定手順｜Ubiquiti UniFi ｜Ubiquiti Japan (UI Japan)](https://note.com/ui_japan/n/n3154ff641db5)

- [1,000円のルーターにOpenWrtをインストールしDS-Liteで使えるようにする | Wi-Fiマニュアル](https://wifi-manual.net/openwrt-dslite-settings/)
- [NICなしx86ミニPCでOpenWrtルータ化して、DS-LiteとPPPoEを同時運用する高速インターネット環境を作った話 #openwrt - Qiita](https://qiita.com/hamasan05/items/45d76a6ec7fa08932f6c)
- [OpenWrtでIPv6とDS-LiteとPPPoEを全部使う - nabeの雑記帳](https://nabe.adiary.jp/0633)

- [OpenWrt DS-LITE 各VNE事業者別 全自動構成 #openwrt - Qiita](https://qiita.com/site_u/items/b6d5097f5e3a0f91c95d)

- [1,000円のルーターにOpenWrtをインストールしDS-Liteで使えるようにする | Wi-Fiマニュアル](https://wifi-manual.net/openwrt-dslite-settings/)
- [OpenWrtを使用したds-lite | 暇言](https://wp.hima-jin.info/openwrt-ds-lite/)

- [LinuxルーターでのDS-Lite設定 | ktaka's blog](https://ktaka.blog.ccmp.jp/2020/05/linuxds-lite.html/)
